...
def myFileRead0(filePath, commonShape, dtype=np.uint8, trimPercent=None, uFirstSlice=None, uLastSlice=None):
	...
	if os.path.isfile(filePath):
		...
		if trimPercent is not None and trimPercent > 0:
			...
			stackData = aicsUtil.trimStack(stackData, trimPercent)
...
def myGetBlock(aicsGridParam, channel, finalPostfixStr, getAll=False):
	"""
		...
		for col in range(nCol):
			...
			if file is not None:
				...
				if useMasterFile:
					...
					uFile, tmp_uInclude, uFirstSlice, uLastSlice = aicsUtil.parseMasterFile('', fileNoPostfix, dfMasterFile=dfMasterFile)
...
def aicsGridNapari(aicsGridParam):
	"""
	...
	for channel in channelList:
		for finalPostfixStr in finalPostfixList:
			...
			print(f'  {blockNum} of {numBlocks} calling myGetBlock() channel "{finalPostfixStr}"')
	...
	with napari.gui_qt():
		...
		def viewerMouseDrag(viewer, event):
			...
			print('  xCoord (um):', xCoord, 'yCoord (um):', yCoord)
			...
			print('  Row:', myRow, 'Col:', myCol, 'File Idx:', myIdx)
